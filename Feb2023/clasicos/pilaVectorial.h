#ifndef PILAVECTORIAL_H_
#define PILAVECTORIAL_H_
...
template <typename tElemento> class PilaVec
{
    ...
    private:
        ...
        int tope_;  //tope de la pila (posición)
};

template <typename tElemento>
    inline PilaVec<tElemento>::PilaVec(unsigned TamaMax) :
    elementos(new tElemento[TamaMax], Lmax(TamaMax), tope_(-1)) {}

template <typename tElemento>
    PilaVec<tElemento>::PilaVec(const PilaVec<tElemento>& P) :
...
template <typename tElemento>
    inline bool PilaVec<tElemento>::llena() const {return (tope_ == Lmax -1); }
